...
def build_performance_features(df):
    # 1. Get each swimmer's best time per year, and grab their Age AND Country
    # We group by FINA ID to ensure one record per athlete per year
    yearly_best = df.groupby(['FINA ID', 'Swimmer', 'Year']).agg({
        'Time_Sec': 'min',
        'Age': 'max',
        'Country': 'first'  # <--- ADDED: Keep the country data
    }).reset_index()
    ...
    for fina_id, group in yearly_best.groupby('FINA ID'):
        ...
        career_stats.append({
            ...
            'latest_gap_to_top10': latest_year_data['gap_to_top8']
        })

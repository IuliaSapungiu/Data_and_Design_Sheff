...
# 5. MAIN CONTENT (Wrapped in your perfect column structure)
...
with main_col:
    ...
    # --- ADVANCED LOGIC INTEGRATION STARTS HERE ---
    ...
    def process_and_navigate(swimmer_name, event_name, full_filtered_df):
        with st.spinner(f"Crunching engine data for {swimmer_name}..."):
            ...
            if target_data.empty:
                ...
                st.session_state['analytics_loaded'] = True # Ensures Navbar renders links
    ...
    if not swimmer_summary.empty:
        ...
        def highlight_rows(row):
            ...
            if rank == 1:
                color = 'background-color: rgba(255, 215, 0, 0.15);' 
            elif rank == 2:
                color = 'background-color: rgba(192, 192, 192, 0.15);' 
            elif rank == 3:
                color = 'background-color: rgba(205, 127, 50, 0.15);' 
            else:
                if total_rows > 3:
